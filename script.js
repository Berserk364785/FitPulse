...
const I18N={
  ru:{
    ...
    inviteDesc:'Получите <strong style="color:var(--primary)">+100 XP</strong>, когда друг по вашей ссылке завершит первую тренировку',
    lbPrivacy:'🔒 Фото-аватары видны только вам в профиле — в общую таблицу лидеров публикуется эмодзи для приватности',
    ...
    publishBtn:'🌍 Опубликовать мой результат',
    // настройки
    langLabel:'Language / Язык',voiceCoachDiv:'🎙️ Голосовой тренер',
    calibBtn:'📏 Калибровка',testVoice:'🔊 Проверить голос',
    // видео
    videoTitle:'🎥 Видео-анализ',videoWip:'Ещё в разработке',
    videoWipDesc:'Анализ загруженных видео пока не работает стабильно. Мы дорабатываем эту функцию и оповестим вас, когда она будет готова. Пока используйте режим «📷 Камера» — он полностью рабочий.',
    // аватар
    avatarUpload:'Нажмите чтобы загрузить фото',avatarOrEmoji:'— или выберите эмодзи —',
    // обратная связь
    feedbackLabel:'Ваш отзыв или идея',feedbackRating:'Оценка',sendFeedback:'📨 Отправить отзыв',
    feedbackNote:'Отзывы сохраняются и помогают делать FitPulse лучше',
    contactDirect:'📬 Связаться напрямую',
    contactDesc:'Есть идея для приложения или нашли баг? Напишите на почту проекта:',
    contactEmail:'✉️ Написать на почту',
    // прочее
  ...
  en:{
    ...
    inviteDesc:'Get <strong style="color:var(--primary)">+100 XP</strong> when a friend completes their first workout via your link',
    lbPrivacy:'🔒 Photo avatars are visible only to you — the leaderboard uses emoji for privacy',
    notifDesc:'Used to notify when your workout timer ends.',
    updateDesc:'If the interface looks outdated, press the button below to clear cache and load a fresh version.',
    publishBtn:'🌍 Publish my result',
    // settings
    langLabel:'Language / Язык',voiceCoachDiv:'🎙️ Voice Coach',
    calibBtn:'📏 Calibrate',testVoice:'🔊 Test voice',
    // video
    videoTitle:'🎥 Video Analysis',videoWip:'Still in development',
    videoWipDesc:'Video upload analysis is not yet stable. We are working on it and will notify you when it\'s ready. For now, use 📷 Camera mode — it works fully.',
    // avatar
    avatarUpload:'Tap to upload a photo',avatarOrEmoji:'— or choose an emoji —',
    // feedback
    feedbackLabel:'Your review or idea',feedbackRating:'Rating',sendFeedback:'📨 Send feedback',
    feedbackNote:'Reviews are saved and help improve FitPulse',
    contactDirect:'📬 Contact directly',
    contactDesc:'Have an idea or found a bug? Email the project:',
    contactEmail:'✉️ Send email',
    // misc
...
function applyLanguage(lang){
  ...
  // Элементы с innerHTML (содержат HTML-теги, нельзя textContent)
  document.querySelectorAll('[data-i18n="inviteDesc"],[data-i18n="lbPrivacy"]').forEach(el=>{
    el.innerHTML=t(el.dataset.i18n);
  });
  // Перерисовываем динамические блоки
  buildExGrid();buildMoreExGrid();buildProgsGrid();
  updateAchUI();updatePRList();updateLvlUI();updateQuestUI();updateChallengeUI();
  // Обновляем hub-changelog кнопку (содержит span)
  const hubChangelogBtn=q('hubChangelogBtn');
  if(hubChangelogBtn)hubChangelogBtn.innerHTML=`${t('hubChangelog')} <span class="menu-hub-sub">${t('hubChangelogSub')}</span>`;
  const hubSettingsBtn=q('hubSettingsBtn');
  if(hubSettingsBtn)hubSettingsBtn.innerHTML=`${t('hubSettings')} <span class="menu-hub-sub">${t('hubSettingsSub')}</span>`;
  const hubFaqBtn=q('hubFaqBtn');
  if(hubFaqBtn)hubFaqBtn.innerHTML=`${t('hubFaq')} <span class="menu-hub-sub">${t('hubFaqSub')}</span>`;
  const hubFeedbackBtn=q('hubFeedbackBtn');
  if(hubFeedbackBtn)hubFeedbackBtn.innerHTML=`${t('hubFeedback')} <span class="menu-hub-sub">${t('hubFeedbackSub')}</span>`;
...
// ============================================================
const EX={
  pushup:{nameRu:'Отжимания',nameEn:'Push-ups',get name(){return currentLang==='en'?this.nameEn:this.nameRu;},dn:95,up:150,ang:'elbow',cal:.5,xp:10,emoji:'💪',metaRu:'Грудь, трицепс',metaEn:'Chest, triceps',get meta(){return currentLang==='en'?this.metaEn:this.metaRu;}},
  squat:{nameRu:'Приседания',nameEn:'Squats',get name(){return currentLang==='en'?this.nameEn:this.nameRu;},dn:115,up:160,ang:'knee',cal:.7,xp:12,emoji:'🦵',metaRu:'Квадрицепс, ягодицы',metaEn:'Quads, glutes',get meta(){return currentLang==='en'?this.metaEn:this.metaRu;}},
  plank:{nameRu:'Планка',nameEn:'Plank',get name(){return currentLang==='en'?this.nameEn:this.nameRu;},dn:150,up:180,ang:'sha',isPlank:true,calS:.15,xpS:2,emoji:'⏱️',metaRu:'Кор, пресс',metaEn:'Core, abs',get meta(){return currentLang==='en'?this.metaEn:this.metaRu;}},
  situp:{nameRu:'Пресс',nameEn:'Sit-ups',get name(){return currentLang==='en'?this.nameEn:this.nameRu;},dn:55,up:85,ang:'trunk',cal:.4,xp:8,emoji:'🔺',metaRu:'Пресс',metaEn:'Abs',get meta(){return currentLang==='en'?this.metaEn:this.metaRu;}},
  lunge:{nameRu:'Выпады',nameEn:'Lunges',get name(){return currentLang==='en'?this.nameEn:this.nameRu;},dn:105,up:160,ang:'knee',cal:.6,xp:10,emoji:'🏃',metaRu:'Ноги, баланс',metaEn:'Legs, balance',get meta(){return currentLang==='en'?this.metaEn:this.metaRu;}},
  burpee:{nameRu:'Бёрпи',nameEn:'Burpees',get name(){return currentLang==='en'?this.nameEn:this.nameRu;},dn:95,up:150,ang:'elbow',cal:1.0,xp:20,emoji:'🤸',metaRu:'Всё тело',metaEn:'Full body',get meta(){return currentLang==='en'?this.metaEn:this.metaRu;}},
  pullup:{nameRu:'Подтягивания',nameEn:'Pull-ups',get name(){return currentLang==='en'?this.nameEn:this.nameRu;},dn:130,up:50,ang:'elbow',cal:.8,xp:15,emoji:'🧗',metaRu:'Спина, бицепс',metaEn:'Back, biceps',get meta(){return currentLang==='en'?this.metaEn:this.metaRu;}}
};
...
// ============================================================
const PROGS=[
  {id:'beginner',titleRu:'Начинающий',titleEn:'Beginner',get title(){return currentLang==='en'?this.titleEn:this.titleRu;},emoji:'🌱',metaRu:'7 дней • базовый',metaEn:'7 days • basic',get meta(){return currentLang==='en'?this.metaEn:this.metaRu;},steps:[
  ...
  {id:'strength',titleRu:'Сила',titleEn:'Strength',get title(){return currentLang==='en'?this.titleEn:this.titleRu;},emoji:'💪',metaRu:'5 дней • интенсивный',metaEn:'5 days • intense',get meta(){return currentLang==='en'?this.metaEn:this.metaRu;},steps:[
    {ex:'pushup',reps:15,sets:4},{ex:'pullup',reps:5,sets:3},{ex:'squat',reps:20,sets:4},{ex:'lunge',reps:12,sets:3}]},
  {id:'cardio',titleRu:'Кардио',titleEn:'Cardio',get title(){return currentLang==='en'?this.titleEn:this.titleRu;},emoji:'🏃',metaRu:'4 дня • сжигание',metaEn:'4 days • burn',get meta(){return currentLang==='en'?this.metaEn:this.metaRu;},steps:[
    {ex:'burpee',reps:10,sets:3},{ex:'squat',reps:20,sets:3},{ex:'lunge',reps:15,sets:3}]},
  {id:'core',titleRu:'Кор & Пресс',titleEn:'Core & Abs',get title(){return currentLang==='en'?this.titleEn:this.titleRu;},emoji:'🔥',metaRu:'6 дней • пресс',metaEn:'6 days • abs',get meta(){return currentLang==='en'?this.metaEn:this.metaRu;},steps:[
...
function updateLvlUI(){
  ...
  q('profileTitle').textContent=`${currentLang==='en'?'Level':'Уровень'} ${lvl} • ${lvlTitle()}`;
  ...
  if(eff){
    ...
    eff.textContent=currentLang==='en'
      ?(pctEff>=100?'⚡ Base XP per exercise: 100%':`⚡ Base XP: ${pctEff}% — vary exercises for bonus`)
      :(pctEff>=100?'⚡ Базовый XP за упражнение: 100%':`⚡ Базовый XP за упражнение: ${pctEff}% — чередуйте упражнения для бонуса`);
  }
}
function lvlTitle(){
  const ru=['Новичок','Атлет','Боец','Чемпион','Легенда'];
  const en=['Beginner','Athlete','Fighter','Champion','Legend'];
  const arr=currentLang==='en'?en:ru;
  return arr[Math.min(Math.floor(lvl/2),arr.length-1)];
}
...
async function showHistory(){
  ...
  else el.innerHTML=h.map(r=>`<div class="hist-item"><div class="hist-date">${r.date||''}</div><div class="hist-main">${EX[r.exercise]?.emoji||''} ${r.exName||r.exercise}: ${r.reps} ${currentLang==='en'?'reps':'повт.'}</div><div class="hist-sub">🔥 ${r.calories||0} ${currentLang==='en'?'cal':'кал'}</div></div>`).join('');
...
// ============================================================
const ACH=[
  {k:'first',  ru:'Первое повторение', en:'First rep',       e:'🎯',c:()=>repCount>=1},
  {k:'ten',    ru:'10 за подход',      en:'10 in a set',     e:'💪',c:()=>repCount>=10},
  {k:'fifty',  ru:'50 всего',          en:'50 total',        e:'🔥',c:()=>totalVolume>=50},
  {k:'hundred',ru:'100 всего',         en:'100 total',       e:'💯',c:()=>totalVolume>=100},
  {k:'fiveHundred',ru:'500 всего',     en:'500 total',       e:'🏆',c:()=>totalVolume>=500},
  {k:'plankMaster',ru:'Планка 60с',    en:'Plank 60s',       e:'⏱️',c:()=>plankTime>=60},
  {k:'streak10',ru:'Серия 10',         en:'Streak 10',       e:'⚡',c:()=>streak>=10},
  {k:'streak20',ru:'Серия 20',         en:'Streak 20',       e:'🌟',c:()=>streak>=20},
  {k:'cal100', ru:'100 калорий',       en:'100 calories',    e:'🔥',c:()=>caloriesBurned>=100},
  {k:'lvl5',   ru:'Уровень 5',         en:'Level 5',         e:'⭐',c:()=>lvl>=5},
  {k:'lvl10',  ru:'Уровень 10',        en:'Level 10',        e:'🌠',c:()=>lvl>=10},
  {k:'variety',ru:'5 упражнений',      en:'5 exercises',     e:'🎭',c:()=>Object.keys(prRecords).length>=5},
];
function achName(a){return currentLang==='en'?a.en:a.ru;}
function checkAch(){
  let nu=false;
  ACH.forEach(a=>{if(!achievements[a.k]&&a.c()){achievements[a.k]=true;speak(`${currentLang==='en'?'Achievement':'Ачивка'}: ${achName(a)}`);bAch();confetti(1500);toast(`🏅 ${achName(a)}`);nu=true;}});
...
function updatePRList(){
  ...
  el.innerHTML=keys.map(k=>`<div class="pr-row"><span>${EX[k]?.emoji||''} ${EX[k]?.name||k}</span><span class="pr-val">${prRecords[k]} ${currentLang==='en'?'reps':'повт.'}</span></div>`).join('');
...
function renderSteps(){
  q('progStepsList').innerHTML=activeProg.steps.map((s,i)=>`<div class="step-row${i===progIdx?' cur':i<progIdx?' done':''}"><span>${EX[s.ex].emoji}</span><span>${EX[s.ex].name} — ${s.reps} × ${s.sets} ${currentLang==='en'?'sets':'подх.'}</span></div>`).join('');
}
...
async function refreshTeamRoomUI(){
  ...
  q('teamRoomMembers').innerHTML=room.members.map((m,i)=>`<div class="lb-item${m.device_id===myId?' me':''}">
    ...
    <div class="lb-score">${m.progress} ${currentLang==='en'?'reps':'повт.'}</div>
  </div>`).join('');
}
...
function openTab(id){
  ...
  if(el){
    ...
    // Перезапускаем анимацию slide-in
...
const CHANGELOG=[
  {v:7,date:'Август 2026',items:[
    ...
    '🏆 Победитель получает +50 XP и уведомление о результате',
    '🌊 Animated gradient фон — живой пульсирующий фон под любую тему',
    '✨ Ripple-эффект — на всех кнопках и карточках при нажатии',
    '🎞️ Slide-анимация вкладок — плавное появление при переключении',
    '💜 Shimmer на XP-баре — блик скользит по прогресс-бару',
    '🔥 Streak-пульсация — баннер серии светится если тренируешься 3+ дня подряд',
  ]},
...
// ============================================================
//  DUELS — система дуэлей через Supabase Realtime
// ============================================================
let duelState = null; // null | 'incoming' | 'waiting' | 'active' | 'finished'
let duelData  = {};   // { id, challengerId, challengerName, opponentId, opponentName, exercise, duration }
let duelTimer = null;
let duelSub   = null; // Supabase Realtime subscription
let duelMyScore    = 0;
let duelOpponentScore = 0;
let duelTimeLeft = 0;

const DUEL_DURATION = 60; // секунд

// Получаем Supabase клиент из cloud.js
...
async function subscribeToIncomingDuels(){
  const sb=getSupabaseClient();if(!sb)return;
  const myId=localStorage.getItem('fp_user_id');if(!myId)return;
  try{
    if(duelSub){await sb.removeChannel(duelSub);duelSub=null;}
    duelSub=sb.channel('duels-incoming-'+myId)
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'fp_duels',filter:`opponent_id=eq.${myId}`},
        payload=>{
          const d=payload.new;
          if(d.status==='pending'){
            duelData={id:d.id,challengerId:d.challenger_id,challengerName:d.challenger_name,
                      opponentId:d.opponent_id,opponentName:d.opponent_name,
                      exercise:d.exercise,duration:d.duration||DUEL_DURATION};
            duelState='incoming';
            refreshDuelUI();
            // Пуш-уведомление если приложение в фоне
            if(Notification.permission==='granted'&&document.visibilityState!=='visible'){
              new Notification('⚔️ Вызов на дуэль!',{
                body:`${d.challenger_name} вызывает тебя на дуэль! ${EX[d.exercise]?.name||d.exercise}`,
                icon:'./icon-192.png'
              });
            }
      ...
      .on('postgres_changes',{event:'UPDATE',schema:'public',table:'fp_duels'},
        payload=>{
          const d=payload.new;
          if(duelData.id&&d.id===duelData.id){
            // Обновляем счёт соперника в реальном времени
            const myId2=localStorage.getItem('fp_user_id');
            if(d.challenger_id===myId2)duelOpponentScore=d.opponent_score||0;
            else duelOpponentScore=d.challenger_score||0;
      ...
      .subscribe();
  }catch(e){console.warn('Duel subscription failed',e);}
}

// ── Вызвать игрока ─────────────────────────────────────────
async function challengePlayer(opponentId,opponentName){
  const sb=getSupabaseClient();if(!sb){toast('☁️ Облако недоступно');return;}
  const myId=localStorage.getItem('fp_user_id');
  if(!myId){toast('❌ Сначала опубликуй результат в рейтинге');return;}
  if(duelState){toast('⚔️ Ты уже в дуэли');return;}

  const exercise=currentEx||'pushup';
  const {data,error}=await sb.from('fp_duels').insert({
    challenger_id:myId, challenger_name:userName,
    opponent_id:opponentId, opponent_name:opponentName,
    exercise, duration:DUEL_DURATION,
    status:'pending',
    challenger_score:0, opponent_score:0,
    created_at:new Date().toISOString()
  }).select().single();

  if(error||!data){toast('❌ Не удалось отправить вызов');return;}
  duelData={id:data.id,challengerId:myId,challengerName:userName,
             opponentId,opponentName,exercise,duration:DUEL_DURATION};
  duelState='waiting';
  refreshDuelUI();
...
async function acceptDuel(){
  const sb=getSupabaseClient();if(!sb||!duelData.id)return;
  const{error}=await sb.from('fp_duels').update({status:'active'}).eq('id',duelData.id);
  if(error){toast('❌ Ошибка принятия дуэли');return;}
  duelState='active';
  refreshDuelUI();
  startDuelTimer();
  toast('⚔️ Дуэль началась! Поехали!');
  speakCoachLine('start');
  startEncourageLoop();
}
...
async function declineDuel(){
  const sb=getSupabaseClient();if(!sb||!duelData.id)return;
  await sb.from('fp_duels').update({status:'declined'}).eq('id',duelData.id);
  duelState=null;duelData={};
  refreshDuelUI();
  toast('Дуэль отклонена');
}
...
function startDuelTimer(){
  ...
  // Переключаем упражнение если нужно
  if(duelData.exercise&&duelData.exercise!==currentEx)setEx(duelData.exercise);
}

// ── Добавить очко в дуэли (вызывается из addRep) ───────────
async function addDuelScore(){
  if(duelState!=='active')return;
  duelMyScore++;
  updateDuelScoreUI();
  // Обновляем свой счёт в Supabase
  const sb=getSupabaseClient();if(!sb||!duelData.id)return;
  const myId=localStorage.getItem('fp_user_id');
  const field=duelData.challengerId===myId?'challenger_score':'opponent_score';
  await sb.from('fp_duels').update({[field]:duelMyScore}).eq('id',duelData.id);
}
...
async function finishDuel(iAmFinisher){
  ...
  if(iAmFinisher){
    ...
    if(sb&&duelData.id)await sb.from('fp_duels').update({status:'finished'}).eq('id',duelData.id);
  }
  duelState='finished';
  const won=duelMyScore>duelOpponentScore;
  const tied=duelMyScore===duelOpponentScore;
  if(won){speak(currentLang==='en'?'You won the duel!':'Ты выиграл дуэль!','!');confetti(3000);bRecord();}
  else if(tied){speak(currentLang==='en'?'It\'s a tie!':'Ничья!','!');}
  else{speak(currentLang==='en'?'Good fight! Next time!':'Хорошая борьба! В следующий раз!','coach');}
  if(won){const bonus=50;addXP(bonus);toast(`🏆 Победа в дуэли! +${bonus} XP`);}
  refreshDuelUI();
...
function updateDuelScoreUI(){
  q('duelMyScore')&&(q('duelMyScore').textContent=duelMyScore);
  q('duelOpScore')&&(q('duelOpScore').textContent=duelOpponentScore);
  // Прогресс-бар — лидерство
  const total=duelMyScore+duelOpponentScore||1;
  const pct=Math.round(duelMyScore/total*100);
  if(q('duelProgressFill'))q('duelProgressFill').style.width=pct+'%';
}
...
function refreshDuelUI(){
  ...
  if(!CLOUD_ENABLED){
    host.innerHTML=`<div class="duel-card" style="opacity:.5;text-align:center;font-size:.82rem;color:var(--text2)">⚔️ Дуэли доступны после публикации результата в рейтинге</div>`;
    return;
  }
  if(!duelState){
    // Показываем список игроков для вызова
    renderDuelPlayerList();
    return;
  }
  if(duelState==='incoming'){
    host.innerHTML=`
      <div class="duel-incoming">
        ...
        <div style="font-size:1.1rem;font-weight:900;margin:8px 0">${currentLang==='en'?'Duel Challenge!':'Вызов на дуэль!'}</div>
        <div style="font-size:.88rem;color:var(--text2);margin-bottom:16px">
          <strong>${duelData.challengerName}</strong> ${currentLang==='en'?'challenges you to':'вызывает тебя:'} ${EX[duelData.exercise]?.name||duelData.exercise} • ${duelData.duration}${currentLang==='en'?'s':'с'}
        </div>
        <div style="display:flex;gap:10px;justify-content:center">
          <button class="m-btn" onclick="acceptDuel()" style="background:linear-gradient(135deg,var(--primary),var(--secondary))">⚔️ ${currentLang==='en'?'Accept':'Принять'}</button>
          <button class="btn-sec" onclick="declineDuel()">✕ ${currentLang==='en'?'Decline':'Отклонить'}</button>
        </div>
      </div>`;
    return;
  }
  if(duelState==='waiting'){
    host.innerHTML=`
      <div class="duel-card" style="text-align:center">
        ...
        <div style="font-weight:700">${currentLang==='en'?'Waiting for':'Ждём'} ${duelData.opponentName}…</div>
        <div style="font-size:.78rem;color:var(--text2);margin-top:6px">${currentLang==='en'?'The duel starts when they accept':'Дуэль начнётся когда они примут вызов'}</div>
        <button class="btn-sec" style="margin-top:12px" onclick="declineDuel()">✕ ${currentLang==='en'?'Cancel':'Отмена'}</button>
      </div>`;
    return;
  }
  if(duelState==='active'){
    ...
    host.innerHTML=`
      <div class="duel-card">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
          <span style="font-size:.72rem;font-weight:700;color:var(--text2);text-transform:uppercase">${isEn?'DUEL':'ДУЭЛЬ'} • ${EX[duelData.exercise]?.name||duelData.exercise}</span>
          <span style="font-size:.72rem;color:var(--accent)">LIVE ●</span>
        </div>
        ...
        <div style="font-size:.72rem;color:var(--text2);text-align:center;margin-top:6px">${isEn?'Reps count automatically via camera':'Повторения считаются автоматически камерой'}</div>
      </div>`;
    updateDuelTimerUI();
    updateDuelScoreUI();
    return;
  ...
  if(duelState==='finished'){
    ...
    host.innerHTML=`
      <div class="duel-result duel-card">
        ...
        <div class="duel-result-title" style="color:${won?'var(--primary)':tied?'var(--accent)':'var(--text2)'}">${won?(isEn?'Victory!':'Победа!'):tied?(isEn?'Draw!':'Ничья!'):(isEn?'Good fight!':'Хорошая борьба!')}</div>
        ...
        ${won?`<div class="duel-result-sub">+50 XP ${isEn?'bonus':'бонус за победу'} 🎉</div>`:''}
      </div>`;
  }
}
...
async function renderDuelPlayerList(){
  const host=q('duelHost');if(!host)return;
  const isEn=currentLang==='en';
  host.innerHTML=`<div class="card-title">⚔️ ${isEn?'Challenge a player':'Вызвать игрока'}</div><div id="duelPlayerList"><div style="color:var(--text2);font-size:.82rem;text-align:center;padding:16px">⏳ ${isEn?'Loading players...':'Загружаем игроков...'}</div></div>`;
  const sb=getSupabaseClient();if(!sb)return;
  const myId=localStorage.getItem('fp_user_id');
  try{
    ...
    list.innerHTML=data.map(p=>`
      <div class="duel-invite-row">
        <div>
          <div class="duel-invite-name">${p.name||'Игрок'}</div>
          <div class="duel-invite-meta">Lv.${p.lvl||1} • ${p.xp||0} XP</div>
        </div>
        <button class="duel-challenge-btn" onclick="challengePlayer('${p.user_id}','${(p.name||'Игрок').replace(/'/g,'')}')">⚔️ ${isEn?'Challenge':'Вызвать'}</button>
      </div>`).join('');
  }catch(e){
    const list=q('duelPlayerList');
    if(list)list.innerHTML=`<div style="color:var(--text2);font-size:.82rem;text-align:center;padding:16px">${isEn?'Failed to load players':'Не удалось загрузить игроков'}</div>`;
  }
}

// ── Хук в addRep — считаем очки дуэли ─────────────────────
// (вызывается из addRep после tickCombo)
...
window.onload=()=>{
  ...
  document.addEventListener('pointerdown',e=>{
    const btn=e.target.closest('.m-btn,.btn-start,.btn-sec,.tab-btn,.ex-card,.icon-btn,.duel-challenge-btn');
    if(!btn)return;
  ...
  // ── Streak hot-pulse если серия ≥ 3 дней ───────────────
  if(dayStreak>=3)q('dayStreakBanner')?.classList.add('hot');
